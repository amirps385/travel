...
const categories = computed(() => [
  ...
  ...props.typeOptions.map(name => ({ id: slugify(name), name }))
])
...
/* ---------- Initialize from prop (initialCategory) ---------- */

// apply initialCategory when component mounts
onMounted(() => {
  if (props.initialCategory) {
    const c = String(props.initialCategory).trim()
    if (c) {
      activeCategory.value = c
      filters.category = c
    }
  }
  // emit initial filters so parent can react immediately
  applyFilters()
})

// keep in sync if parent/page changes the prop later (e.g. route query changes)
watch(() => props.initialCategory, (newVal) => {
  const c = String(newVal || 'all').trim()
  activeCategory.value = c
  filters.category = c
  // re-apply to notify parent
  applyFilters()
})
</script>
