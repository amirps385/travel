        ...
        <div class="p-6 md:p-8 lg:p-10">
          ...
          <div v-if="success" class="text-center py-12">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
              <NuxtLink to="/" force class="inline-flex items-center justify-center bg-white/10 hover:bg-white/20 text-white font-medium px-6 py-3 rounded-xl border border-white/20 transition-colors">
              ...
              <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    @change="updateCountrySelection"
                    class="w-full px-4 py-3 bg-white/5 border border-white/20 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 transition appearance-none cursor-pointer"
                  >
                    <option value="" class="bg-gray-800 text-white">Select country</option>
                    <option v-for="c in countries" :key="c.code" :value="c.name" class="bg-gray-800 text-white">
                      {{ c.flag }} {{ c.name }}
                    </option>
                  </select>
                </div>
                ...
                <div>
                  <label for="countryCode" class="block text-sm font-medium text-gray-300 mb-2">
                    Country code <span class="text-red-400">*</span>
                  </label>
                  <input
                    id="countryCode"
            ...
            <div>
              ...
              <div class="mb-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  >
                    <input type="radio" v-model="form.who" value="single" class="sr-only" @change="handleWhoChange" />
                  ...
                  >
                    <input type="radio" v-model="form.who" value="couple" class="sr-only" @change="handleWhoChange" />
                    <div class="text-amber-400 mb-2 group-hover:text-amber-300 transition-colors">
                      <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  >
                    <input type="radio" v-model="form.who" value="groups" class="sr-only" @change="handleWhoChange" />
                    <div class="text-amber-400 mb-2 group-hover:text-amber-300 transition-colors">
                      <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  >
                    <input type="radio" v-model="form.who" value="family" class="sr-only" @change="handleWhoChange" />
                    <div class="text-amber-400 mb-2 group-hover:text-amber-300 transition-colors">
                      <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <!-- Adults -->
                <div>
                  <label class="block text-sm font-medium text-gray-300 mb-2">Adults (18+)</label>
                  <div class="flex items-center gap-3">
                    <button 
                      type="button" 
                      @click="changeAdults(-1)" 
                      class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white/10 transition-colors"
                    >-</button>
                    <div class="w-12 text-center font-bold text-white">{{ form.adults }}</div>
                    <button 
                      type="button" 
                      @click="changeAdults(1)" 
                      class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white/10 transition-colors"
                  ...
                  <div v-if="form.adults > 0" class="mt-3 grid grid-cols-2 gap-2">
                    <div v-for="(age, idx) in form.adultAges" :key="'a'+idx">
                      <select 
                        v-model.number="form.adultAges[idx]" 
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-1 focus:ring-amber-500 appearance-none cursor-pointer"
                      >
                        <option v-for="a in adultAgeOptions" :key="a" :value="a" class="bg-gray-800 text-white">{{ a }}</option>
                      </select>
                    </div>
                  </div>
                </div>
                ...
                <!-- Children -->
                <div>
                  <label class="block text-sm font-medium text-gray-300 mb-2">Children (0-17)</label>
                  <div class="flex items-center gap-3">
                    <button 
                      type="button" 
                      ...
                      class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white/10 transition-colors"
                    >-</button>
                    <div class="w-12 text-center font-bold text-white">{{ form.children }}</div>
                    <button 
                      type="button" 
                      @click="changeChildren(1)" 
                      class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white/10 transition-colors"
                  ...
                  <div v-if="form.children > 0" class="mt-3 grid grid-cols-2 gap-2">
                    <div v-for="(age, idx) in form.childAges" :key="'c'+idx">
                      <select 
                        v-model.number="form.childAges[idx]" 
                        class="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-1 focus:ring-amber-500 appearance-none cursor-pointer"
                      >
                        <option v-for="a in childAgeOptions" :key="a" :value="a" class="bg-gray-800 text-white">{{ a }}</option>
                      </select>
                    </div>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    >
                      <option v-for="tz in timezones" :key="tz" :value="tz" class="bg-gray-800 text-white">{{ tz }}</option>
                    </select>
                  ...
                  <div class="mt-2 text-xs text-gray-400">
                    <p>Detected browser timezone: <span class="text-amber-300">{{ detectedBrowserTimezone }}</span></p>
                    <p>Selected timezone: <span class="text-amber-300">{{ form.timezone }}</span></p>
                  </div>
                </div>
              </div>
...
const detectedBrowserTimezone = ref('Not detected')

// Timezone options - UPDATED to include Asia/Calcutta
const timezones = [
  'Asia/Kolkata',
  'Asia/Calcutta', // Added for compatibility
...
// Country list (kept small like journey.vue)
const countries = [
...
const dateErrors = reactive({ travelDate: '', preferredTime: '' })
const dateValidations = reactive({ travelDate: false, preferredTime: false })
const numberErrors = reactive({ days: '', budget: '' })
const numberValidations = reactive({ days: false, budget: false })
...
const formatPhone = () => {
  ...
  if (form.phone) validateField('phone')
}

// format country code (digits only)
const formatCountryCode = () => {
  const digitsOnly = String(form.countryCode || '').replace(/\D/g, '')
  form.countryCode = digitsOnly ? digitsOnly : ''
  if (form.countryCode) validateField('countryCode')
}

// When user picks a country from dropdown, fill countryCode (digits only)
function updateCountrySelection() {
  ...
  if (selected) {
    // store digits-only code (no plus), consistent with API you showed
    form.countryCode = String(selected.phoneCode || '').replace(/\D/g, '')
    console.log('Country code set to:', form.countryCode)
  }
}

// Log timezone changes
function logTimezoneChange() {
  console.log('Timezone changed by user to:', form.timezone)
  console.log('Browser detected timezone:', detectedBrowserTimezone.value)
}

// Adults / children incrementers
...
// Keep adultAges/childAges arrays in sync with counts
watch(() => form.adults, (v) => {
  ...
  // sync travellers if user hasn't manually edited travellers
  if (!userEditedTravellers.value) {
    form.travellers = Number(form.adults || 0) + Number(form.children || 0)
  }
})
...
// Validation functions (similar to previous)
const validateField = (field) => {
  switch (field) {
    case 'name': return validateName()
    case 'email': return validateEmail()
    case 'countryCode': return validateCountryCode()
    case 'phone': return validatePhone()
    case 'travellers': return validateTravellers()
    case 'budget': return validateBudget()
    case 'preferredTime': return validatePreferredTime()
  }
}

const validateName = () => {
  if (!form.name || !String(form.name).trim()) {
    errors.name = 'Name is required'; validations.name = false; return false
  } else if (form.name.trim().length < 2) {
    errors.name = 'Name must be at least 2 characters'; validations.name = false; return false
  } else { errors.name = ''; validations.name = true; return true }
}

const validateEmail = () => {
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!form.email) { errors.email = 'Email is required'; validations.email = false; return false }
  if (!emailRegex.test(form.email)) { errors.email = 'Please enter a valid email address'; validations.email = false; return false }
  errors.email = ''; validations.email = true; return true
}

const validateCountryCode = () => {
  if (!form.countryCode || !String(form.countryCode).trim()) {
    errors.countryCode = 'Country code is required'; validations.countryCode = false; return false
  }
  const digitsOnly = String(form.countryCode).replace(/\D/g, '')
  if (!/^\d{1,4}$/.test(digitsOnly)) {
    errors.countryCode = 'Please enter a valid country code (1-4 digits)'; validations.countryCode = false; return false
  }
  errors.countryCode = ''; validations.countryCode = true; return true
}

const validatePhone = () => {
  const digits = String(form.phone || '').replace(/\D/g, '')
  if (!digits) { errors.phone = 'Phone number is required'; validations.phone = false; return false }
  if (digits.length < 6) { errors.phone = 'Phone number must be at least 6 digits'; validations.phone = false; return false }
  if (digits.length > 15) { errors.phone = 'Phone number must be 15 digits or less'; validations.phone = false; return false }
  errors.phone = ''; validations.phone = true; return true
}

const validateWho = () => {
  if (!form.who) { errors.who = 'Please select who is travelling'; return false }
  errors.who = ''; return true
}

const validateTravellers = () => {
  if (!form.travellers || form.travellers < 1 || form.travellers > 99) {
    errors.travellers = 'Please enter a valid number of travellers (1-99)'; validations.travellers = false; return false
  }
  errors.travellers = ''; validations.travellers = true; return true
}

const validateBudget = () => {
  if (!form.budget && form.budget !== 0) { errors.budget = 'Budget is required'; validations.budget = false; return false }
  const num = Number(form.budget)
  if (isNaN(num)) { errors.budget = 'Please enter a valid budget'; validations.budget = false; return false }
  if (num < 100) { errors.budget = 'Budget should be at least $100 per person'; validations.budget = false; return false }
  errors.budget = ''; validations.budget = true; return true
}

const validatePreferredTime = () => {
  if (!form.preferredTime) { errors.preferredTime = 'Preferred call date & time is required'; validations.preferredTime = false; return false }
  const date = new Date(form.preferredTime)
  if (isNaN(date.getTime())) { errors.preferredTime = 'Please enter a valid date and time'; validations.preferredTime = false; return false }
  const now = new Date()
  if (date < now) { errors.preferredTime = 'Please select a future date and time'; validations.preferredTime = false; return false }
  errors.preferredTime = ''; validations.preferredTime = true; return true
}

const validateConsent = () => {
  if (!form.consent) { errors.consent = 'You must consent to be contacted'; validations.consent = false; return false }
  errors.consent = ''; validations.consent = true; return true
}

const validateDate = (field) => {
  const value = form[field]
  if (!value) {
    dateErrors[field] = 'Travel date is required'
    dateValidations[field] = false
    return false
  }
  const date = new Date(value)
  if (isNaN(date.getTime())) { dateErrors[field] = 'Please enter a valid date'; dateValidations[field] = false; return false }
  const now = new Date()
  if (date < now) { dateErrors[field] = 'Please select a future travel date'; dateValidations[field] = false; return false }
  dateErrors[field] = ''; dateValidations[field] = true; return true
}

const validateNumber = (field) => {
  const value = form[field]
  if (!value && value !== 0) {
    numberErrors[field] = ''; numberValidations[field] = false; return true
  }
  const num = Number(value)
  if (isNaN(num)) { numberErrors[field] = 'Please enter a valid number'; numberValidations[field] = false; return false }
  if (field === 'days' && num < 1) { numberErrors[field] = 'Trip length must be at least 1 day'; numberValidations[field] = false; return false }
  numberErrors[field] = ''; numberValidations[field] = true; return true
}

const validateForm = () => {
  // If user hasn't manually edited travellers, ensure it's synced
  if (!userEditedTravellers.value) form.travellers = Number(form.adults || 0) + Number(form.children || 0) || 1
...
const submitForm = async () => {
  ...
  console.log('Form timezone being sent:', form.timezone)
  console.log('Form country being sent:', form.country)
  console.log('Detected browser timezone:', detectedBrowserTimezone.value)
  console.log('All form data:', JSON.parse(JSON.stringify(form)))
  console.log('=== END DEBUG ===')

  // Ensure travellers synced if user didn't manually edit
  if (!userEditedTravellers.value) form.travellers = Number(form.adults || 0) + Number(form.children || 0) || 1
  ...
  if (!validateForm()) {
    ...
    setTimeout(() => {
      const el = document.querySelector('.border-red-400, .text-red-400')
  ...
  try {
    ...
    if (form.preferredTime) {
      const d = new Date(form.preferredTime)
      if (!isNaN(d.getTime())) preferredTimeISO = d.toISOString()
    }

    // Build payload to match lead schema (same shape as journey)
    const payload = {
      ...
      preferredContactMethod: '', // bookacall doesn't have explicit pick here (left empty)
      scheduleCall: true,
      activities: [],
      kilimanjaroRoute: '',
      countries: [],
      ...
      dateIsMonthOnly: false,
      adults: Number(form.adults) || 0,
      children: Number(form.children) || 0,
      travellers: Number(form.travellers) || (Number(form.adults || 0) + Number(form.children || 0) || 1),
      adultAges: Array.isArray(form.adultAges) ? form.adultAges.map(a => Number(a)) : [],
      childAges: Array.isArray(form.childAges) ? form.childAges.map(a => Number(a)) : [],
      ages: [].concat(Array.isArray(form.adultAges) ? form.adultAges.map(a => Number(a)) : [], Array.isArray(form.childAges) ? form.childAges.map(a => Number(a)) : []),
      genders: [],
      ...
      metadata: {
        ...
        browserTimezone: detectedBrowserTimezone.value // Add browser timezone to metadata for debugging
      }
    }
    ...
    // gentle reset (preserve timezone)
    ...
    Object.keys(form).forEach(k => {
      ...
      else if (k === 'adults') form[k] = 1
      else if (k === 'children') form[k] = 0
      else if (k === 'adultAges') form[k] = [30]
      else if (k === 'childAges') form[k] = []
      else form[k] = ''
    })
    ...
    else if (typeof err === 'string') error.value = err
    else error.value = 'Failed to submit. Please check your connection and try again.'
    setTimeout(() => {
...
onMounted(() => {
  ...
  try {
    ...
    console.log('Browser detected timezone (raw):', userTz)
    detectedBrowserTimezone.value = userTz || 'Not detected'
    
    if (userTz) {
      // Handle Asia/Calcutta vs Asia/Kolkata equivalence
      if (userTz === 'Asia/Calcutta') {
        console.log('Converting Asia/Calcutta to Asia/Kolkata')
        form.timezone = 'Asia/Kolkata'
      } else if (timezones.includes(userTz)) {
        console.log('Browser timezone found in list, setting to:', userTz)
        form.timezone = userTz
      } else {
        console.log('Browser timezone not in list, trying to find match for:', userTz)
        const tzLower = userTz.toLowerCase()
        const found = timezones.find(tz => tz.toLowerCase().includes(tzLower.split('/')[1]?.toLowerCase() || ''))
        if (found) {
          console.log('Found matching timezone:', found)
          form.timezone = found
        } else {
          console.log('No matching timezone found, keeping default:', form.timezone)
        }
      }
    } else {
    ...
    detectedBrowserTimezone.value = 'Error detecting'
  }

  console.log('Final form timezone after detection:', form.timezone)

  // default preferredTime to local datetime
  const now = new Date()
  const localDateTime = new Date(now.getTime() - now.getTimezoneOffset() * 60000).toISOString().slice(0, 16)
  form.preferredTime = localDateTime

  // travelDate default 30 days from now
  const futureDate = new Date(now)
  futureDate.setDate(futureDate.getDate() + 30)
  form.travelDate = futureDate.toISOString().split('T')[0]

  // Set timezone if possible - already done above
  console.log('Page mounted. Current timezone in form:', form.timezone)
})

// small helpers used in template validations
function validateDateField(field) { return validateDate(field) }
...
/* Custom select arrow - UPDATED FOR DARK MODE */
select {
...
.text-center { animation: fadeIn 0.5s ease-out; }
</style>