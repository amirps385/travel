  ...
  <div class="min-h-screen bg-linear-to-b from-emerald-50/30 via-white to-amber-50/20">
      ...
      <div class="relative z-10 max-w-7xl mx-auto px-4 lg:px-6">
        <div class="text-center max-w-4xl mx-auto">
          ...
          <div class="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 mb-8">
            ...
            <span class="text-sm text-emerald-100 font-medium tracking-wider">
              PROTECTED PARKS OF TANZANIA
            </span>
          </div>
          ...
          <h1 class="text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight">
            Tanzania's
            <span class="text-transparent bg-clip-text bg-linear-to-r from-emerald-300 to-teal-300">
              Wild Crown Jewels
            </span>
          </h1>
          ...
          <div class="flex flex-wrap gap-4 justify-center">
            ...
            <button
              @click="navigateTo('/about#conservation')"
    ...
    <section class="py-20 md:py-28 bg-linear-to-b from-slate-50 to-white">
      <div class="max-w-7xl mx-auto px-4 lg:px-6">
        ...
        <div v-else-if="error" class="text-center py-20">
          <div class="inline-block p-8 bg-linear-to-br from-white to-slate-50 rounded-3xl shadow-xl border border-slate-100">
            ...
            <p class="text-slate-600 max-w-md mx-auto mb-8">
              ...
              If the problem persists, contact support and we'll investigate immediately.
            </p>
            <button
              @click="refresh"
        ...
        <!-- No Parks -->
        <div v-else-if="parks.length === 0" class="text-center py-20">
          <div class="inline-block p-10 bg-linear-to-br from-white to-slate-50 rounded-3xl shadow-xl border border-slate-100 max-w-md">
            ...
            <h3 class="text-2xl font-bold text-slate-900 mb-3">Parks updating</h3>
            <p class="text-slate-600 mb-8">
              Our conservation teams are updating park details. Please check back soon for the latest park listings
              and travel guidance.
            </p>
          </div>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div class="relative h-64 overflow-hidden">
              <img
                :src="park.featuredImage || defaultImage"
                :alt="park.name"
            ...
            <div class="p-6">
              ...
              <p class="text-slate-600 mb-6 line-clamp-2 leading-relaxed">
                {{ park.shortDescription || defaultShortDescription(park) }}
              </p>

              <!-- Action Button -->
              <button
                @click="viewPark(park.slug)"
        ...
        <!-- Park Count Display - Simple & Clean -->
        <div v-if="!pending && parks.length > 0" class="text-center mt-16 pt-12 border-t border-slate-200">
          <div class="mb-8">
            <div class="inline-flex items-center justify-center gap-4 mb-6">
              ...
              <div class="text-lg font-bold text-emerald-700 tracking-wider uppercase">
                TANZANIA'S NATIONAL HERITAGE
              </div>
              <div class="w-12 h-0.5 bg-slate-300"></div>
            </div>
            
            <div class="mb-6">
              <div class="text-5xl md:text-6xl font-bold text-emerald-700 mb-3">
                {{ activeParksCount }}
              </div>
      ...
      <div class="max-w-6xl mx-auto px-4 lg:px-6">
        <div class="text-center">
          ...
          <h2 class="text-4xl md:text-5xl font-bold text-white mb-6">
            Ready for a safari designed for you?
          </h2>
          ...
          <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <button @click="navigateTo('/contact')" class="px-8 py-4 bg-white text-emerald-900 font-bold rounded-xl hover:bg-emerald-50 transition-all duration-300 hover:scale-105 shadow-lg">
              Book Your Safari
            </button>
            <button @click="navigateTo('/itineraries')" class="px-8 py-4 bg-transparent border-2 border-emerald-400 text-white font-bold rounded-xl hover:bg-emerald-400/10 transition-all duration-300">
...
/* ----------------------
  Fetch parks data
  API returns either array or { ok: true, data: [...] }
  ---------------------- */
const { data: parksData, pending, error } = await useFetch('/api/parks')

/* Helper to normalize response shapes (robust) */
...
/* Filter only active parks (public listing) */
const parks = computed(() => {
  const parksArray = extractParksArray(parksData.value)
  return parksArray.filter(park => park?.isActive === true)
})
...
const viewPark = (slug) => {
  if (!slug) return
  navigateTo(`/parks/${slug}`)
}
const refresh = () => {
  // refreshNuxtData triggers useFetch to refetch on the page
  try { refreshNuxtData() } catch (e) { /* noop */ }
}
...
useSeoMeta({
  ...
  ogDescription: 'Explore protected parks known for the Great Migration, big game sightings and diverse ecosystems. Plan a responsible safari with local experts.',
  ogImage: '/images/tanzania-safari-og.jpg',
  twitterCard: 'summary_large_image',
  twitterTitle: 'Tanzania National Parks • Safaris & Wildlife',
  twitterDescription: 'Plan an unforgettable safari across Tanzania\'s most celebrated national parks.',
  keywords: 'Tanzania safari, Serengeti, Ngorongoro, Tarangire, Kilimanjaro, wildlife safari, responsible travel'
})
</script>
