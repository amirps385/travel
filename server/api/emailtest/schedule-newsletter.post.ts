...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!scheduledAt) {
      ...
      debug.error = 'scheduledAt is required'
    ...
    debug.steps.push(`Scheduling newsletter: to=${toEmail}, time=${scheduledAt}`)
    ...
    try {
      ...
      debug.steps.push('Scheduling newsletter email')
      const resp = await api.sendTransacEmail(sendOptions)
      debug.steps.push('Newsletter scheduled')
      ...
      try {
        ...
        debug.steps.push('Scheduled newsletter logged')
      } catch (logError: any) {
    ...
    console.error('Unhandled schedule-newsletter error:', err)
